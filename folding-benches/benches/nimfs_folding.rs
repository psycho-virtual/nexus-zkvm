...
/// Benchmark function for NIMFS folding protocol
fn bench_nimfs_folding() {
    ...
    for &num_constraints in DEFAULT_CONSTRAINT_SIZES.iter() {
        ...
        for _ in 0..10 {
            ...
            // Create witness and instance for linearized CCS (LCCS)
            ...
            // Create io and random evaluation point
            ...
            // Compute evaluations of M_j(z) at point rs
            let z1 = [io.as_slice(), witness1.W.as_slice()].concat();
            let vs1: Vec<CF> = shape.Ms.iter()
                .map(|M| {
                    nexus_nova::ccs::mle::vec_to_mle(
                        M.multiply_vec(&z1).as_slice()
            ...
            // Create witness and instance for regular CCS
        ...
        let (proof, _folded_instance, _rho) = proof_result.unwrap();
