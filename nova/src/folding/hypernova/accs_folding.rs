    ...
    RO: CryptographicSponge,
{
    ...
    pub fn prove_as_subprotocol<C: PolyCommitmentScheme<G>>(
        ...
        let (sumcheck_f_proof, sumcheck_f_state) = MLSumcheck::prove_as_subprotocol(random_oracle, &f);
        ...
        let (sumcheck_g_proof, _sumcheck_g_state) = MLSumcheck::prove_as_subprotocol(random_oracle, &g);
...
mod tests {
    ...
    use ark_std::{test_rng, Zero, One, ops::Neg};
    ...
    fn setup_test_instances<G: SWCurveConfig, C: PolyCommitmentScheme<Projective<G>>>(
        shape: &CCSShape<Projective<G>>,
        ck: &C::PolyCommitmentKey,
        rng: &mut impl ark_std::rand::Rng,
    ) -> Result<(ACCSInstance<Projective<G>, C>, CCSWitness<Projective<G>>, CCSInstance<Projective<G>, C>, CCSWitness<Projective<G>>), ccs::Error>
    where
        G::ScalarField: UniformRand,
    {
        // Create ACCS instance
        let W_accs = CCSWitness::zero(shape);
        let commitment_W_accs = W_accs.commit::<C>(ck);
        
        // Generate random values for ACCS
        let v0 = G::ScalarField::rand(rng);
        let io = to_field_elements::<Projective<G>>(&[1, 2]);
        let s_x = ark_std::log2(shape.num_constraints) as usize;
        let r_x: Vec<G::ScalarField> = (0..s_x).map(|_| G::ScalarField::rand(rng)).collect();
        let s_y = ark_std::log2(shape.num_vars) as usize;
        let r_y: Vec<G::ScalarField> = (0..s_y).map(|_| G::ScalarField::rand(rng)).collect();
        ...
        // Create ACCS instance
        let accs = ACCSInstance::<Projective<G>, C>::new(
            &commitment_W_accs,
            &v0,
            &io,
            &r_x,
        ...
        // Create CCS instance
        let W_ccs = CCSWitness::zero(shape);
        let commitment_W_ccs = W_ccs.commit::<C>(ck);
        let X = to_field_elements::<Projective<G>>(&[1, 3]);
        
        let ccs = CCSInstance::<Projective<G>, C>::new(
            shape,
            &commitment_W_ccs,
            &X,
        )?;
        
        Ok((accs, W_accs, ccs, W_ccs))
    }
