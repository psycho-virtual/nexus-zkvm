...
#[instrument(target = LOG_TARGET, skip(input))]
pub fn generate_sha256_leaf_data(
    input: Vec<u8>,
    num_iterations: usize,
) -> super::SHA256ChainMangroveComputation {
    info!(
        target: LOG_TARGET,
    ...
    super::SHA256ChainMangroveComputation::new(input, final_output, hashes, num_iterations)
}
...
mod tests {
    ...
    fn test_generate_sha256_leaf_data() {
        ...
        assert_eq!(
            leaf_data.final_output,
            leaf_data.intermediate_hashes.last().unwrap().clone()
        );
