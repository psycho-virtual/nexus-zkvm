...
use ark_crypto_primitives::sponge::{constraints::SpongeWithGadget, Absorb};
...
where
    ...
    RO::Var: ark_crypto_primitives::sponge::constraints::CryptographicSpongeVar<
        G1::ScalarField,
        RO,
        Parameters = RO::Config,
    >,
    ...
    F: FnMut(&[G1::ScalarField]) -> Vec<G1::ScalarField>,
{
    ...
    println!(
        "[{}] Building binary PCD tree for {} steps (padded to {})",
    ...
    for i in 0..padded_n {
        ...
        println!(
            "[{}] Creating leaf node for step {}",
            LOG_TARGET, step_index
        );
        let leaf_prove_start = Instant::now();

        let leaf_node = PCDNode::prove_leaf(params, step_circuit, step_index, &states[step_index])?;

        let leaf_prove_duration = leaf_prove_start.elapsed();
        total_leaf_prove_time += leaf_prove_duration;
        println!(
            "[{}] Leaf node for step {} created in {:?}",
    ...
    println!(
        "[{}] Created {} leaf nodes in {:?} (avg: {:?} per node)",
        LOG_TARGET, padded_n, leaf_nodes_duration, avg_leaf_prove_time
    );

    // Build the tree bottom-up
    println!(
        "[{}] Building inner nodes of the tree bottom-up",
        LOG_TARGET
    );
    ...
    while level_indices.len() > 1 {
        ...
        for i in 0..(level_indices.len() / 2) {
            ...
            println!(
                "[{}] Creating parent node for children at indices {} and {}",
    ...
    println!(
        "[{}] Created {} inner nodes in {:?} (avg: {:?} per node)",
...
mod tests {
        ...
        C2: CommitmentScheme<Projective<G2>, SetupAux = ()>,
    {
        ...
        let compute_step = |z: &[G1::ScalarField]| {
            let x = z[0];
            // Calculate x^3 manually since cube() is not available
            let result = x * x * x - x + G1::ScalarField::from(5);
            vec![result]
        };

        // Build the binary PCD tree
        let root = build_binary_pcd_tree(&params, &circuit, &initial_input, steps, compute_step)?;
