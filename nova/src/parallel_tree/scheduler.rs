...
const LOG_TARGET: &str = "nexus-nova::parallel_tree::scheduler::leaf_producer";
...
impl<L: Send + Sync + Clone + Debug + 'static> LeafProducer<L> {
    ...
    pub fn spawn(
        ...
        std::thread::spawn(move || {
            ...
            for leaf in self.stream {
                ...
                tracing::info!(
                    target = LOG_TARGET,
                    "Adding leaf {} to worker {}, leaf value: {:?}",
                    next_leaf_idx,
                    worker_idx,
                    leaf
                );
                ...
                loop {
                    if leaf_queues[worker_idx]
                        .push((leaf.clone(), next_leaf_idx))
                        .is_ok()
                    {
                        break;
                    }
    ...
    Error: Send + Sync + Debug + 'static,
{
    ...
    pub fn with_workers(
        ...
        for id in 0..num_cores {
            ...
            let handle = std::thread::Builder::new()
                ...
                .spawn(move || {
                    tracing::info!("Created worker thread: {}", thread_name);
    ...
    pub fn spawn_leaf_producer(&self, stream: Box<dyn Iterator<Item = L> + Send>) {
        ...
        producer.spawn(
            self.task_heap.clone(),
            self.leaf_queues.clone(),
            self.stop.clone(),
        );
    }
    ...
    pub fn is_computation_complete(&self) -> bool {
        self.stop.load(std::sync::atomic::Ordering::Acquire) == 1
    }
...
mod tests {
    ...
    fn scheduler_sums_four_leaf_nodes() {
        ...
        let scheduler =
            Scheduler::<LeafPayload, LeafPayload, DummyFoldProof, ()>::new(pool, 2, reducer);

        // Feed leaf data through producer
        let leaf_stream = Box::new(
            [
                LeafPayload(1),
                LeafPayload(2),
                LeafPayload(3),
    ...
    fn scheduler_sums_two_leaf_nodes() {
        ...
        let scheduler =
            Scheduler::<LeafPayload, LeafPayload, DummyFoldProof, ()>::new(pool, 1, reducer);
    ...
    fn scheduler_stress_test_sixteen_leaves() {
        ...
        let scheduler = Scheduler::<LeafPayload, LeafPayload, DummyFoldProof, ()>::with_workers(
            pool, 4, reducer, 4,
        );
        ...
        if let Some(result) = scheduler.get_root_result() {
            assert_eq!(
                result,
                LeafPayload(expected_sum),
                "expected sum of 1..=16 = {}",
                expected_sum
            );
        } else {
            panic!("no root result available");
