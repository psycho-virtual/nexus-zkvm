...
/// draws them from Poseidon, and fills a 52×5 matrix.
///
/// # Returns
...
///
/// # Returns
/// A 52×5 matrix of Boolean circuit variables representing the derived bits
#[tracing::instrument(
    target = LOG_TARGET,
...
pub fn derive_split_bits_circuit<F>(
...
mod tests {
    ...
    fn test_derive_split_bits_dimensions() {
        ...
        let (bits_mat, num_samples) = derive_split_bits(seed);
    ...
    fn test_derive_split_bits_deterministic() {
        ...
        let (bits_mat1, num_samples1) = derive_split_bits(seed);
        let (bits_mat2, num_samples2) = derive_split_bits(seed);
    ...
    fn test_derive_split_bits_different_seeds() {
        ...
        let (bits_mat1, _) = derive_split_bits(seed1);
        let (bits_mat2, _) = derive_split_bits(seed2);
    ...
    fn test_derive_split_bits_circuit_dimensions() {
        ...
        let bits_mat = derive_split_bits_circuit(cs.clone(), &seed_var, num_samples)
    ...
    fn test_derive_split_bits_circuit_native_consistency() {
        ...
        // Native version
        let (native_bits, num_samples) = derive_split_bits(seed_field);

        // Circuit version using the same number of samples
        let cs = ConstraintSystem::<TestField>::new_ref();
        let seed_var =
            FpVar::new_input(cs.clone(), || Ok(seed_field)).expect("Failed to allocate seed");
        let circuit_bits = derive_split_bits_circuit(cs.clone(), &seed_var, num_samples)
            .expect("Circuit execution failed");
    ...
    fn test_derive_split_bits_circuit_deterministic() {
        ...
        let bits1 = derive_split_bits_circuit(cs1.clone(), &seed_var1, num_samples)
            .expect("Circuit execution failed");
        ...
        let bits2 = derive_split_bits_circuit(cs2.clone(), &seed_var2, num_samples)
            .expect("Circuit execution failed");
    ...
    fn test_derive_split_bits_circuit_constraint_count() {
        ...
        let seed_var = FpVar::new_input(cs.clone(), || Ok(seed)).expect("Failed to allocate seed");
        let _bits_mat = derive_split_bits_circuit(cs.clone(), &seed_var, num_samples)
            .expect("Circuit execution failed");
