...
mod tests {
    ...
    #[test]
    fn test_poseidon_hash_2_points() -> Result<(), Box<dyn std::error::Error>> {
        ...
        init_tracing();
        tracing::info!(target: TEST_TARGET, "Starting Poseidon hash test with 10 rounds");

        // Create a timing tracker
        use std::collections::HashMap;
        let mut timings = HashMap::new();

        // Using the same curve setup as the shuffle circuit
        type BaseField = <GrumpkinProjective as CurveGroup>::BaseField; // BN254's Fr
        ...
        let config = poseidon_config::<BaseField>();
        ...
        let seed_value = BaseField::from(42u64);
        ...
        for round in 0..num_rounds {
            ...
            let dummy_eval1 = BaseField::from((round * 2) as u64);
            let dummy_eval2 = BaseField::from((round * 2 + 1) as u64);
            native_sponge.absorb(&vec![dummy_eval1, dummy_eval2]);

            // Squeeze random value
            let random_value: BaseField = native_sponge.squeeze_field_elements(1)[0];
        ...
        let seed_var = FpVar::<BaseField>::new_input(cs.clone(), || Ok(seed_value))?;
        ...
        // Simulate 10 rounds like sumcheck
        let num_rounds = 1;
        ...
        for round in 0..num_rounds {
            ...
            let dummy_evals = vec![
                FpVar::<BaseField>::new_witness(cs.clone(), || {
                    Ok(BaseField::from((round * 2) as u64))
                })?,
                FpVar::<BaseField>::new_witness(cs.clone(), || {
                    Ok(BaseField::from((round * 2 + 1) as u64))
                })?,
            ];
